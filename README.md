...
说明：在任何时间点，用户获得但还尚未分配的 ERC20 数量为pendingReward = (user.amount * pool.accERC20PerShare) - user.rewardDebt。每当用户向池中存入或提取 LP 代币时，会发生以下情况：

- 更新池的 `accERC20PerShare`（和 `lastRewardBlock`）。
- 用户收到发送到其地址的待分配奖励。
- 用户的 `amount` 被更新。
- 用户的 `rewardDebt` 被更新。
- 定义池子信息结构（PoolInfo）：
